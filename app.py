...
with page_tabs[1]:
    ...
    with opt_tabs[0]:
        ...
        with oc3:
            ...
            st.markdown(f"""<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;padding-top:8px;">
                ...
                SPOT ₹{spot_val:,.2f} | {dte_val}D</span></div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""<div style="background:var(--surface);border:1px solid {uc};border-radius:8px;padding:12px 16px;margin:10px 0;display:flex;gap:16px;align-items:center;flex-wrap:wrap;">
            <div><div style="font-family:Orbitron;font-size:1rem;color:{uc};">{chain_idx}: {u_rec}</div>
            <div style="font-size:0.75rem;color:var(--muted);">Strength {u_str}% | {'Prefer CE (BUY CALLS)' if 'BUY' in u_rec else ('Prefer PE (BUY PUTS)' if 'SELL' in u_rec else 'Sideways — Straddle/Strangle')}</div></div>
            {''.join([f'<span style="background:var(--surface);border:1px solid var(--border);border-radius:4px;padding:2px 8px;font-size:0.65rem;color:var(--text2);">{r[:55]}</span>' for r in u_rsn[:3]])}
        </div>""", unsafe_allow_html=True)
        ...
        if st.button(f"🔄 Load {chain_idx} Chain ({dte_val}D to Expiry)", use_container_width=False, key="load_chain"):
        ...
        if chain_data:
            st.markdown(f'<div class="info-b" style="font-size:0.72rem;">Chain loaded at {chain_ts} · {len(chain_data)} strikes · Black-Scholes + Live VIX ({vix_val:.1f})</div>', unsafe_allow_html=True)
            ...
            st.markdown("""<div class="oc-hdr">
                ...
                <div style="color:var(--gold);text-align:center;font-family:Orbitron;font-size:0.75rem;letter-spacing:1px">STRIKE</div>
            ...
            strike_list = [f"{r['strike']:,} {'🎯ATM' if r['is_atm'] else r['type']}" for r in chain_data]
            ...
            for col, otype, color in [(dd1,"CE","var(--accent)"),(dd2,"PE","var(--red)")]:
                with col:
                    ...
                    t1   = sel_row[f"{otype.lower()}_t1"]
                    t2   = sel_row[f"{otype.lower()}_t2"]
                    t3   = sel_row[f"{otype.lower()}_t3"]
                    ...
                    sig  = sel_row[f"{otype.lower()}_signal"]

                    st.markdown(f"""<div style="background:var(--card);border:1px solid {color};border-radius:10px;padding:14px;">
                    <div style="font-family:Orbitron;font-size:1.1rem;color:{color};letter-spacing:2px;margin-bottom:8px;">
                    ...
                    # Signal
                    st.markdown(f"<div style='margin-top:8px;'>{oc_sig_html(sig)} <span style='font-size:0.72rem;color:var(--muted);'>Strength: {sig['strength']}%</span></div>",unsafe_allow_html=True)
                    for r in sig["reasons"][:4]:
                        st.markdown(f"<div style='font-size:0.7rem;color:var(--text2);padding:1px 0;'>• {r}</div>",unsafe_allow_html=True)
                    ...
                    if pr>0 and "BUY" in sig["signal"]:
                        ...
                        st.markdown(f'<div class="info-b" style="font-size:0.72rem;">Kelly: {lots} lot(s) | Invested: ₹{pr*lots*lot_size:,.0f} | Charges: ₹{cost:.2f}</div>',unsafe_allow_html=True)
                        if st.button(f"🚀 BUY {sel_row['strike']} {otype}", key=f"dd_buy_{otype}_{sel_row['strike']}"):
                            trade = {
                                ...
                                "index":chain_idx,"strike":sel_row["strike"],"type":otype,
                                "expiry":str(exp_date),"entry":pr,"cmp":pr,
                                "lots":lots,"lot_size":lot_size,
                                "invested":round(pr*lots*lot_size,2),"brokerage":cost,
                                "sl":sl,"t1":t1,"t2":t2,"t3":t3,
                                "trailing_sl":None,"pnl":0.0,"status":"OPEN",
                                "entry_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                "entry_dt":datetime.now().isoformat(),
                                "signal":sig["signal"],"strength":sig["strength"],
                                "delta":dlt,"theta":tht,"dte":sel_row["dte"],
                            }
    ...
    with opt_tabs[1]:
        ...
        with osc1:
            scan_indices = st.multiselect("Scan", ["BANKNIFTY","NIFTY50"], default=["BANKNIFTY","NIFTY50"], key="opt_scan_idx")
        with osc2:
                    ...
                    for row in chain_s:
                        for ot in ["CE","PE"]:
                            ...
                            if "BUY" in sig_d["signal"] and sig_d["strength"]>=opt_min_str2 and pr_val>0:
                                lot = row["lot"]
                                all_sigs.append({
                                    "index":idx_name,"strike":row["strike"],"type":ot,
                                    "expiry":str(exp),"price":pr_val,
                                    "sl":row[f"{ot.lower()}_sl"],
                                    "t1":row[f"{ot.lower()}_t1"],"t2":row[f"{ot.lower()}_t2"],"t3":row[f"{ot.lower()}_t3"],
                                    "delta":row[f"{ot.lower()}_delta"],"gamma":row[f"{ot.lower()}_gamma"],
                                    "theta":row[f"{ot.lower()}_theta"],"vega":row[f"{ot.lower()}_vega"],
                                    "iv":row["iv"],"lot":lot,"dte":row["dte"],
                                    "signal":sig_d["signal"],"strength":sig_d["strength"],
                                    "score":sig_d["score"],"reasons":sig_d["reasons"],
                                    "is_atm":row["is_atm"],
                                })
        ...
        if opt_type_filter=="CE Only":   scan_opt=[s for s in scan_opt if s["type"]=="CE"]
        elif opt_type_filter=="PE Only": scan_opt=[s for s in scan_opt if s["type"]=="PE"]
        elif opt_type_filter=="STRONG Only": scan_opt=[s for s in scan_opt if "STRONG" in s["signal"]]

        if scan_opt:
            sm=st.columns(5)
            ce_c=[s for s in scan_opt if s["type"]=="CE"]; pe_c=[s for s in scan_opt if s["type"]=="PE"]
            ...
            for s in scan_opt:
                tbl_opt.append({
                    ...
                    "Delta":f"{s['delta']:.3f}","Theta":f"{s['theta']:.2f}","IV%":f"{s['iv']}%",
                    "DTE":s["dte"],"ATM":"🎯" if s.get("is_atm") else ""
                })
            ...
            for sig in scan_opt[:30]:
                ...
                lot = sig["lot"]
                with st.expander(f"{'🔵' if sig['type']=='CE' else '🔴'} {sig['index']} {sig['strike']:,} {sig['type']}{atm_tag} | ₹{sig['price']:.2f} | {sig['signal']} {sig['strength']}% | Δ={sig['delta']:.3f}"):
                    ...
                    st.markdown(f'<div class="info-b" style="font-size:0.72rem;">Kelly: {lots2} lot(s) | ₹{sig["price"]*lots2*lot:,.0f} invested | Charges: ₹{cost2:.2f}</div>',unsafe_allow_html=True)

                    if st.button(f"🚀 BUY {sig['index']} {sig['strike']} {sig['type']}", key=f"scan_opt_buy_{sig['index']}_{sig['strike']}_{sig['type']}"):
                        trade = {
                            ...
                            "lots":lots2,"lot_size":lot,"invested":round(sig["price"]*lots2*lot,2),
                            "brokerage":cost2,"sl":sig["sl"],"t1":sig["t1"],"t2":sig["t2"],"t3":sig["t3"],
                            "trailing_sl":None,"pnl":0.0,"status":"OPEN",
                            "entry_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "entry_dt":datetime.now().isoformat(),
                            "signal":sig["signal"],"strength":sig["strength"],
    ...
    with opt_tabs[2]:
        ...
        if vix_val > 28:
            st.markdown('<div class="warn-b">🚨 VIX > 28 — Options auto trading BLOCKED. Too dangerous for option buying.</div>',unsafe_allow_html=True)

        if not st.session_state.auto_opt:
            st.markdown("""<div style="background:var(--surface);border:1px solid var(--accent);border-radius:10px;padding:18px;text-align:center;margin-bottom:14px;">
            <div style="font-family:Orbitron;font-size:1.2rem;color:var(--accent);letter-spacing:3px;">AI OPTIONS AUTO TRADER</div>
            <div style="color:var(--text2);font-size:0.8rem;margin-top:6px;">Scans BankNifty + Nifty50 · All Strikes · Delta-filtered · Staged Profit Booking · Auto Trailing SL</div>
            </div>""",unsafe_allow_html=True)
            ...
            with oa2:
                ...
                oa_idx = st.multiselect("Trade Indices",["BANKNIFTY","NIFTY50"],default=["BANKNIFTY","NIFTY50"],key="oa_idx")
                oa_str = st.number_input("Min signal strength",50,95,60,5,key="oa_str")
                bias_map = {"BULLISH":"CE","BEARISH":"PE","NEUTRAL":"Both"}
                st.markdown(f'<div class="info-b">Market Bias: <b>{mood}</b> → Prefer <b>{bias_map.get(mood,"Both")}</b> | VIX: {vix_val:.1f}</div>',unsafe_allow_html=True)
                if vix_val<=28:
                    if st.button("🚀 START OPTIONS AUTO TRADING",use_container_width=True,key="oa_start"):
                        st.session_state.auto_opt = True
                        st.session_state.auto_opt_end = (datetime.now()+timedelta(minutes=int(oa_dur))).isoformat()
                        st.session_state["oa_max2"]=int(oa_max); st.session_state["oa_idx2"]=oa_idx; st.session_state["oa_str2"]=int(oa_str)
                        db.save("auto_opt",True); db.save("auto_opt_end",st.session_state.auto_opt_end)
                        st.rerun()
                else:
            ...
            if rem<=0:
                for pos in st.session_state.opt_portfolio:
                    ep=pos["entry"]; cmp=pos.get("cmp",ep); lots=pos["lots"]; ls=pos["lot_size"]
                    gross=(cmp-ep)*lots*ls; net=gross-pos.get("brokerage",0)
                    st.session_state.opt_history.append({**pos,"exit":cmp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                    st.session_state.journal.append({"cat":"OPTIONS","symbol":f"{pos['index']}{pos['strike']}{pos['type']}","pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("signal","")})
                st.session_state.opt_portfolio=[]; st.session_state.auto_opt=False
                db.save("auto_opt",False); db.save("opt_portfolio",[]); db.save("opt_history",st.session_state.opt_history); db.save("journal",st.session_state.journal)
                update_kelly(); st.rerun()
            else:
                _max=st.session_state.get("oa_max2",3); _idxs=st.session_state.get("oa_idx2",["BANKNIFTY"]); _str=st.session_state.get("oa_str2",60)
                if len(st.session_state.opt_portfolio)<_max and vix_val<=28:
                    with st.spinner("Scanning options chains…"):
                        new_sigs=[]
                        for iname in _idxs:
                            sp=bn.get("p",50000) if iname=="BANKNIFTY" else nf.get("p",22000)
                            ex=exp_bn if iname=="BANKNIFTY" else exp_nf
                            ch=eng.build_chain(iname,sp,ex,vix_val,8)
                            for row in ch:
                                for ot in ["CE","PE"]:
                                    if mood=="BULLISH" and ot=="PE": continue
                                    if mood=="BEARISH" and ot=="CE": continue
                                    sg=row[f"{ot.lower()}_signal"]; pr2=row[f"{ot.lower()}_price"]
                                    if "BUY" in sg["signal"] and sg["strength"]>=_str and pr2>0:
                                        new_sigs.append({"index":iname,"strike":row["strike"],"type":ot,"expiry":str(ex),"price":pr2,"sl":row[f"{ot.lower()}_sl"],"t1":row[f"{ot.lower()}_t1"],"t2":row[f"{ot.lower()}_t2"],"t3":row[f"{ot.lower()}_t3"],"delta":row[f"{ot.lower()}_delta"],"theta":row[f"{ot.lower()}_theta"],"lot":row["lot"],"dte":row["dte"],"signal":sg["signal"],"strength":sg["strength"]})
                    ...
                    for sig in new_sigs:
                        ...
                        trade={"id":f"{sig['index']}{sig['strike']}{sig['type']}_{int(time.time()*1000)}","index":sig["index"],"strike":sig["strike"],"type":sig["type"],"expiry":sig["expiry"],"entry":pr2,"cmp":pr2,"lots":lots2,"lot_size":lot,"invested":round(pr2*lots2*lot,2),"brokerage":cost2,"sl":sig["sl"],"t1":sig["t1"],"t2":sig["t2"],"t3":sig["t3"],"trailing_sl":None,"pnl":0.0,"status":"OPEN","entry_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"entry_dt":datetime.now().isoformat(),"signal":sig["signal"],"strength":sig["strength"],"delta":sig["delta"],"theta":sig["theta"],"dte":sig["dte"]}
                        st.session_state.opt_portfolio.append(trade); existing.add(k)

                # Update + exit check
                still=[]
                for pos in st.session_state.opt_portfolio:
                    ep=pos["entry"]; lots=pos["lots"]; ls=pos["lot_size"]
                    cmp=pos.get("cmp",ep)
                    gross=(cmp-ep)*lots*ls; pos["pnl"]=round(gross-pos.get("brokerage",0),2)
                    ...
                    hit=(cmp<=eff_sl or cmp>=pos.get("t3",ep*3))
                    if use_time_x:
                        try:
                            ed2=datetime.fromisoformat(pos.get("entry_dt",datetime.now().isoformat()))
                            if (datetime.now()-ed2).total_seconds()>1800 and abs(pnl_pct)<10: hit=True
                        except: pass
                    if hit:
                        cost3=eng.options_cost(cmp,lots,ls,"SELL"); gross2=(cmp-ep)*lots*ls; net=gross2-pos.get("brokerage",0)-cost3
                        st.session_state.opt_history.append({**pos,"exit":cmp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        st.session_state.journal.append({"cat":"OPTIONS","symbol":f"{pos['index']}{pos['strike']}{pos['type']}","pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("signal","")})
                    else:
                        still.append(pos)
                st.session_state.opt_portfolio=still
                db.save("opt_portfolio",still); db.save("opt_history",st.session_state.opt_history); db.save("journal",st.session_state.journal)
                update_kelly()

                st.markdown("### Live Options Positions")
                if st.session_state.opt_portfolio:
                    for pos in st.session_state.opt_portfolio:
                        oc2=f"var(--accent)" if pos["type"]=="CE" else "var(--red)"
                        trail2=f" | Trail SL: ₹{pos['trailing_sl']:.2f}" if pos.get("trailing_sl") else ""
                        pn=pos.get("pnl",0)
                        st.markdown(f"""<div class="tc {'win' if pn>=0 else 'loss'}">
                        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;">
                            <span style="font-family:Orbitron;font-size:0.9rem;color:{oc2};">{pos['index']} {pos['strike']:,} {pos['type']}</span>
                            <span class="tc-meta">Entry ₹{pos['entry']:.2f} | CMP ₹{pos.get('cmp',pos['entry']):.2f} | {pos['lots']}L{trail2}</span>
                            {pnl_fmt(pn)}
                        </div></div>""",unsafe_allow_html=True)
                else:
                ...
                with stp2:
                    if st.button("🛑 STOP OPTIONS AUTO TRADING",key="opt_stop",use_container_width=True):
                        for pos in st.session_state.opt_portfolio:
                            ep=pos["entry"]; cmp2=pos.get("cmp",ep); lots=pos["lots"]; ls=pos["lot_size"]
                            gross=(cmp2-ep)*lots*ls; net=gross-pos.get("brokerage",0)-eng.options_cost(cmp2,lots,ls,"SELL")
                            st.session_state.opt_history.append({**pos,"exit":cmp2,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                            st.session_state.journal.append({"cat":"OPTIONS","symbol":f"{pos['index']}{pos['strike']}{pos['type']}","pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("signal","")})
                        st.session_state.opt_portfolio=[]; st.session_state.auto_opt=False
                        db.save("auto_opt",False); db.save("opt_portfolio",[]); db.save("opt_history",st.session_state.opt_history); db.save("journal",st.session_state.journal)
                        update_kelly(); st.rerun()
    ...
    with opt_tabs[3]:
        ...
        if not st.session_state.opt_portfolio:
            ...
            for pos in st.session_state.opt_portfolio:
                ...
                ep=pos["entry"]; lots=pos["lots"]; ls=pos["lot_size"]; pn=pos.get("pnl",0)
                trail3=f" | Trail SL: ₹{pos['trailing_sl']:.2f}" if pos.get("trailing_sl") else ""
                with st.expander(f"{'🔵' if pos['type']=='CE' else '🔴'} {pos['index']} {pos['strike']:,} {pos['type']} | Entry ₹{ep:.2f} | {pos['lots']}L | {pnl_fmt(pn)}{trail3}"):
                    p1,p2,p3,p4,p5 = st.columns(5)
                    p1.metric("Entry",   f"₹{ep:.2f}")
                    p2.metric("CMP",     f"₹{pos.get('cmp',ep):.2f}")
                    p3.metric("SL",      f"₹{pos.get('sl',0):.2f}")
                    p4.metric("T1",      f"₹{pos.get('t1',0):.2f}")
                    p5.metric("Net P&L", f"₹{pn:+,.0f}")

                    st.markdown("**Profit Booking Targets**")
                    st.markdown(profit_book_row(30,pos.get("t1",0),"Book 1/3",(pos.get("t1",0)-ep)*ls*lots),unsafe_allow_html=True)
                    st.markdown(profit_book_row(60,pos.get("t2",0),"Book 1/3 more",(pos.get("t2",0)-ep)*ls*lots),unsafe_allow_html=True)
                    st.markdown(profit_book_row(100,pos.get("t3",0),"Full exit",(pos.get("t3",0)-ep)*ls*lots),unsafe_allow_html=True)

                    if st.button("✅ Square Off",key=f"opt_sq_{pos['id']}"):
                        cmp3=pos.get("cmp",ep); gross3=(cmp3-ep)*lots*ls; cost4=eng.options_cost(cmp3,lots,ls,"SELL"); net2=gross3-pos.get("brokerage",0)-cost4
                        st.session_state.opt_history.append({**pos,"exit":cmp3,"pnl":round(net2,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        st.session_state.journal.append({"cat":"OPTIONS","symbol":f"{pos['index']}{pos['strike']}{pos['type']}","pnl":round(net2,2),"win":net2>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("signal","")})
                        st.session_state.opt_portfolio=[p2b for p2b in st.session_state.opt_portfolio if p2b["id"]!=pos["id"]]
                        db.save("opt_portfolio",st.session_state.opt_portfolio); db.save("opt_history",st.session_state.opt_history); db.save("journal",st.session_state.journal)
                        update_kelly(); st.success(f"Squared off ₹{net2:+,.0f}"); st.rerun()
    ...
    with opt_tabs[4]:
        ...
        if not oh:
            ...
            ow=len([x for x in oh if x.get("pnl",0)>=0]); on=sum(x.get("pnl",0) for x in oh); owr=ow/len(oh)*100
            hc2=st.columns(4)
            hc2[0].metric("Total",len(oh)); hc2[1].metric("Wins",ow); hc2[2].metric("Win Rate",f"{owr:.1f}%"); hc2[3].metric("Net P&L",f"₹{on:+,.0f}")
            ...
            st.dataframe(df_oh[dcols].rename(columns={"entry":"Entry(₹)","exit":"Exit(₹)","pnl":"Net P&L(₹)"}),use_container_width=True,hide_index=True)
            if len(oh)>=2:
                df_oh2=pd.DataFrame(oh); df_oh2["cum"]=df_oh2["pnl"].cumsum()
                fig2=go.Figure(); fig2.add_trace(go.Scatter(y=df_oh2["cum"],mode="lines+markers",line=dict(color="#00e5ff",width=2),fill="tozeroy",fillcolor="rgba(0,229,255,0.06)",marker=dict(color=["#00e676" if p>=0 else "#ff1744" for p in df_oh2["pnl"]],size=7)))
                fig2.update_layout(title="Options Cumulative P&L",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=250,margin=dict(l=40,r=20,t=30,b=20))
                st.plotly_chart(fig2,use_container_width=True)
            st.download_button("📥 Download Options CSV",data=df_oh.to_csv(index=False),file_name=f"options_history_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
            if st.button("🗑️ Clear Options History",key="clr_opt_hist"):
                st.session_state.opt_history=[]; db.save("opt_history",[]); st.rerun()
...
with page_tabs[2]:
    ...
    with fut_tabs[0]:
        ...
        with fc1:
            fut_scan_n = st.number_input("Stocks to scan",30,len(eng.FUTURES_SYMBOLS),len(eng.FUTURES_SYMBOLS),10,key="fut_scan_n")
        with fc2:
        ...
        if fut_filter=="LONG Only":    fut_results=[r for r in fut_results if "BUY" in r["rec"]]
        elif fut_filter=="SHORT Only": fut_results=[r for r in fut_results if "SELL" in r["rec"]]
        elif fut_filter=="STRONG Only":fut_results=[r for r in fut_results if "STRONG" in r["rec"]]

        if fut_results:
            fl=[r for r in fut_results if "BUY" in r["rec"]]; fs=[r for r in fut_results if "SELL" in r["rec"]]
            fm=st.columns(4)
            ...
            fm[3].markdown(metric_card(int(np.mean([r["strength"] for r in fut_results])),"Avg Str%","var(--gold)"),unsafe_allow_html=True)
            st.markdown("<br>",unsafe_allow_html=True)
            ...
            for r in fut_results:
                ...
                ftbl.append({"Symbol":r["symbol"].replace(".NS",""),"Signal":r["rec"],"Str%":r["strength"],"CMP":f"₹{r['price']:,.2f}","Target":f"₹{r['target']:,.2f}","SL":f"₹{r['sl']:,.2f}","R/R":f"{r['rr']:.2f}","5D%":f"{r.get('m5',0):+.1f}%","RSI":f"{r.get('rsi',0):.0f}","ADX":f"{r.get('adx',0):.0f}","Vol":f"{r.get('vr',1):.1f}x","Margin≈":f"₹{margin:,.0f}"})
            ...
            for r in fut_results[:20]:
                ...
                with st.expander(f"{icon} {r['symbol'].replace('.NS','')} | ₹{r['price']:,.2f} | {r['rec']} {r['strength']}% | ADX:{r.get('adx',0):.0f}"):
                    d1,d2,d3,d4,d5 = st.columns(5)
                    d1.metric("CMP",    f"₹{r['price']:,.2f}")
                    d2.metric("Target", f"₹{r['target']:,.2f}")
                    d3.metric("SL",     f"₹{r['sl']:,.2f}")
                    ...
                    if r.get("patterns"):
                        phtml2=" ".join([f'<span style="background:rgba(213,0,249,0.1);border:1px solid rgba(213,0,249,0.3);color:var(--purple);border-radius:3px;padding:1px 6px;font-size:0.7rem;">{p[0]}</span>' for p in r["patterns"]])
                    ...
                    lots_f = max(1, int(kc_f/(margin2 if margin2>0 else 1)))
                    cost_f = eng.futures_cost(r["price"],lots_f,lot_sz2,"BUY")
                    st.markdown(f'<div class="info-b" style="font-size:0.72rem;">Margin/lot: ₹{margin2:,.0f} | Kelly lots: {lots_f} | Charges: ₹{cost_f:.2f}</div>',unsafe_allow_html=True)

                    if r["rec"] not in ("NEUTRAL",):
                        if st.button(f"🚀 {'LONG' if 'BUY' in r['rec'] else 'SHORT'} {r['symbol'].replace('.NS','')} FUTURES",key=f"fut_exec_{r['symbol']}"):
                            trade_f = {
                                "id":f"{r['symbol']}_FUT_{int(time.time()*1000)}",
                                "symbol":r["symbol"],"type":"LONG" if "BUY" in r["rec"] else "SHORT",
                                "entry":r["price"],"cmp":r["price"],"lots":lots_f,"lot_size":lot_sz2,
                                "margin":round(margin2*lots_f,2),"brokerage":cost_f,
                                "target":r["target"],"sl":r["sl"],"trailing_sl":None,"pnl":0.0,
                            ...
                            st.success(f"✅ {'LONG' if 'BUY' in r['rec'] else 'SHORT'} {r['symbol']} Futures @ ₹{r['price']:.2f}")
        else:
    ...
    with fut_tabs[1]:
        ...
        if not st.session_state.auto_fut:
            st.markdown("""<div style="background:var(--surface);border:1px solid var(--purple);border-radius:10px;padding:18px;text-align:center;margin-bottom:14px;">
            <div style="font-family:Orbitron;font-size:1.2rem;color:var(--purple);letter-spacing:3px;">AI FUTURES AUTO TRADER</div>
            <div style="color:var(--text2);font-size:0.8rem;margin-top:6px;">Scans All Equity + Index Futures · Momentum + Breakout + Trend · Kelly Margin Sizing · Auto SL</div>
            </div>""",unsafe_allow_html=True)
            ...
            with fa2:
                ...
                fa_scan=st.number_input("Stocks to scan",10,len(eng.FUTURES_SYMBOLS),len(eng.FUTURES_SYMBOLS),5,key="fa_scan")
                st.markdown(f'<div class="info-b">Market Bias: <b>{mood}</b> | VIX: {vix_val:.1f}</div>',unsafe_allow_html=True)
                if st.button("🚀 START FUTURES AUTO TRADING",use_container_width=True,key="fa_start"):
                    st.session_state.auto_fut=True
                    st.session_state.auto_fut_end=(datetime.now()+timedelta(minutes=int(fa_dur))).isoformat()
                    st.session_state["fa_max2"]=int(fa_max); st.session_state["fa_str2"]=int(fa_str); st.session_state["fa_scan2"]=int(fa_scan)
                    db.save("auto_fut",True); db.save("auto_fut_end",st.session_state.auto_fut_end)
                    st.rerun()
        else:
            end_ft=datetime.fromisoformat(st.session_state.auto_fut_end)
            rem_f=max(0.0,(end_ft-datetime.now()).total_seconds())
            tot_f=max(rem_f,1); prog_f=1.0-rem_f/tot_f
            ...
            if rem_f<=0:
                for pos in st.session_state.fut_portfolio:
                    ep=pos["entry"]; cmp=pos.get("cmp",ep); lots=pos["lots"]; ls=pos["lot_size"]
                    gross=(cmp-ep)*lots*ls if pos["type"]=="LONG" else (ep-cmp)*lots*ls
                    net=gross-pos.get("brokerage",0)
                    st.session_state.fut_history.append({**pos,"exit":cmp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                    st.session_state.journal.append({"cat":"FUTURES","symbol":pos["symbol"],"pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
                st.session_state.fut_portfolio=[]; st.session_state.auto_fut=False
                db.save("auto_fut",False); db.save("fut_portfolio",[]); db.save("fut_history",st.session_state.fut_history); db.save("journal",st.session_state.journal)
                update_kelly(); st.rerun()
            else:
                _fmax=st.session_state.get("fa_max2",3); _fstr=st.session_state.get("fa_str2",60); _fscan=st.session_state.get("fa_scan2",len(eng.FUTURES_SYMBOLS))
                if len(st.session_state.fut_portfolio)<_fmax:
                    ...
                    for sig in fnew:
                        ...
                        p3=sig["price"]; lot_sz3=25 if "NIFTY" in sig["symbol"] else (15 if "BANK" in sig["symbol"] else 1)
                        ...
                        trade_f2={"id":f"{sig['symbol']}_FUT_{int(time.time()*1000)}","symbol":sig["symbol"],"type":"LONG" if "BUY" in sig["rec"] else "SHORT","entry":p3,"cmp":p3,"lots":lots_f2,"lot_size":lot_sz3,"margin":round(margin3*lots_f2,2),"brokerage":cost_f2,"target":sig["target"],"sl":sig["sl"],"trailing_sl":None,"pnl":0.0,"rec":sig["rec"],"strength":sig["strength"],"rr":sig["rr"],"reasons":sig["reasons"][:5],"entry_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"entry_dt":datetime.now().isoformat(),"patterns":[p4[0] for p4 in sig.get("patterns",[])]}
                        st.session_state.fut_portfolio.append(trade_f2); fexist.add(fk)

                fstill=[]
                for pos in st.session_state.fut_portfolio:
                    lp2=eng.get_live_price(pos["symbol"]) or pos["entry"]; pos["cmp"]=lp2
                    ...
                    if use_trail and abs(pnl_pct2)>=1.5:
                        if pos.get("trailing_sl") is None: pos["trailing_sl"]=ep2
                            ...
                            if pos["type"]=="LONG": new_t2=lp2-1.5*atr2; pos["trailing_sl"]=max(pos["trailing_sl"],round(new_t2,2))
                            else: new_t2=lp2+1.5*atr2; pos["trailing_sl"]=min(pos["trailing_sl"],round(new_t2,2))
                    eff_sl2=pos.get("trailing_sl") or pos.get("sl",0)
                    hit2=((pos["type"]=="LONG" and (lp2>=pos.get("target",lp2+1) or lp2<=eff_sl2)) or (pos["type"]=="SHORT" and (lp2<=pos.get("target",0) or lp2>=eff_sl2)))
                    if use_time_x:
                        try:
                            ed3=datetime.fromisoformat(pos.get("entry_dt",datetime.now().isoformat()))
                            if (datetime.now()-ed3).total_seconds()>1800 and abs(pnl_pct2)<0.5: hit2=True
                        except: pass
                    if hit2:
                        cost6=eng.futures_cost(lp2,lots2,ls2,pos["type"]); net3=gross2-cost5-cost6
                        st.session_state.fut_history.append({**pos,"exit":lp2,"pnl":round(net3,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        st.session_state.journal.append({"cat":"FUTURES","symbol":pos["symbol"],"pnl":round(net3,2),"win":net3>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
                    else:
                        fstill.append(pos)
                st.session_state.fut_portfolio=fstill
                db.save("fut_portfolio",fstill); db.save("fut_history",st.session_state.fut_history); db.save("journal",st.session_state.journal)
                update_kelly()
                ...
                if st.session_state.fut_portfolio:
                    for pos in st.session_state.fut_portfolio:
                        ...
                        pn2=pos.get("pnl",0)
                        st.markdown(f"""<div class="tc {'win' if pn2>=0 else 'loss'}">
                        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;">
                            <span style="font-family:Orbitron;font-size:0.9rem;color:{pc3};">{pos['type']} {pos['symbol'].replace('.NS','')} FUT</span>
                            <span class="tc-meta"> Entry ₹{pos['entry']:.2f} | CMP ₹{pos.get('cmp',pos['entry']):.2f} | {pos['lots']}L{trail4}</span>
                            {pnl_fmt(pn2)}
                        </div></div>""",unsafe_allow_html=True)
                else:
                ...
                with fs2:
                    if st.button("🛑 STOP FUTURES AUTO TRADING",key="fut_stop",use_container_width=True):
                        for pos in st.session_state.fut_portfolio:
                            lp3=eng.get_live_price(pos["symbol"]) or pos["entry"]; ep3=pos["entry"]; lots3=pos["lots"]; ls3=pos["lot_size"]
                            gross3=(lp3-ep3)*lots3*ls3 if pos["type"]=="LONG" else (ep3-lp3)*lots3*ls3
                            cost7=eng.futures_cost(lp3,lots3,ls3,pos["type"]); net4=gross3-pos.get("brokerage",0)-cost7
                            st.session_state.fut_history.append({**pos,"exit":lp3,"pnl":round(net4,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                            st.session_state.journal.append({"cat":"FUTURES","symbol":pos["symbol"],"pnl":round(net4,2),"win":net4>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
                        st.session_state.fut_portfolio=[]; st.session_state.auto_fut=False
                        db.save("auto_fut",False); db.save("fut_portfolio",[]); db.save("fut_history",st.session_state.fut_history); db.save("journal",st.session_state.journal)
                        update_kelly(); st.rerun()
    ...
    with fut_tabs[2]:
        ...
        if not st.session_state.fut_portfolio:
            ...
            st.markdown("<br>",unsafe_allow_html=True)
            for pos in st.session_state.fut_portfolio:
                lp4=eng.get_live_price(pos["symbol"]) or pos["entry"]; pos["cmp"]=lp4
                ...
                pos["pnl"]=round(gross4-cost8,2); pn3=pos["pnl"]
                trail5=f" | Trail: ₹{pos['trailing_sl']:.2f}" if pos.get("trailing_sl") else ""
                with st.expander(f"{'🟢' if pos['type']=='LONG' else '🔴'} {pos['type']} {pos['symbol'].replace('.NS','')} FUT | Entry ₹{ep4:.2f} | CMP ₹{lp4:.2f} | {pnl_fmt(pn3)}{trail5}"):
                    fp3_c=st.columns(5)
                    ...
                    if st.button("✅ Square Off",key=f"fut_sq_{pos['id']}"):
                        cost9=eng.futures_cost(lp4,lots4,ls4,pos["type"]); net5=gross4-cost8-cost9
                        st.session_state.fut_history.append({**pos,"exit":lp4,"pnl":round(net5,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                        st.session_state.journal.append({"cat":"FUTURES","symbol":pos["symbol"],"pnl":round(net5,2),"win":net5>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
                        st.session_state.fut_portfolio=[p5 for p5 in st.session_state.fut_portfolio if p5["id"]!=pos["id"]]
                        db.save("fut_portfolio",st.session_state.fut_portfolio); db.save("fut_history",st.session_state.fut_history); db.save("journal",st.session_state.journal)
                        update_kelly(); st.success(f"Squared off ₹{net5:+,.0f}"); st.rerun()
            db.save("fut_portfolio",st.session_state.fut_portfolio)
    ...
    with fut_tabs[3]:
        ...
        if not fh2:
            ...
            fw=len([x for x in fh2 if x.get("pnl",0)>=0]); fn=sum(x.get("pnl",0) for x in fh2); fwr=fw/len(fh2)*100
            fhc=st.columns(4)
            fhc[0].metric("Total",len(fh2)); fhc[1].metric("Wins",fw); fhc[2].metric("Win Rate",f"{fwr:.1f}%"); fhc[3].metric("Net P&L",f"₹{fn:+,.0f}")
            ...
            st.dataframe(df_fh[fdcols].rename(columns={"entry":"Entry(₹)","exit":"Exit(₹)","pnl":"Net P&L(₹)"}),use_container_width=True,hide_index=True)
            if len(fh2)>=2:
                df_fh2=pd.DataFrame(fh2); df_fh2["cum"]=df_fh2["pnl"].cumsum()
                fig3=go.Figure(); fig3.add_trace(go.Scatter(y=df_fh2["cum"],mode="lines+markers",line=dict(color="#d500f9",width=2),fill="tozeroy",fillcolor="rgba(213,0,249,0.05)",marker=dict(color=["#00e676" if p>=0 else "#ff1744" for p in df_fh2["pnl"]],size=7)))
                fig3.update_layout(title="Futures Cumulative P&L",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=250,margin=dict(l=40,r=20,t=30,b=20))
                st.plotly_chart(fig3,use_container_width=True)
            st.download_button("📥 Download Futures CSV",data=df_fh.to_csv(index=False),file_name=f"futures_history_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
            if st.button("🗑️ Clear Futures History",key="clr_fut_hist"):
                st.session_state.fut_history=[]; db.save("fut_history",[]); st.rerun()
...
with page_tabs[3]:
    ...
    all_open  = ([(p,"EQUITY")  for p in st.session_state.eq_portfolio] +
    ...
    if not all_open:
        ...
        total_inv_all = (sum(p.get("invested",0) for p in st.session_state.eq_portfolio) +
        ...
        if total_inv_all > 0:
            ...
            if st.session_state.eq_portfolio:
                seg_labels.append("Equity"); seg_vals.append(sum(p.get("invested",0) for p in st.session_state.eq_portfolio)); seg_colors.append("#00e676")
            if st.session_state.opt_portfolio:
                seg_labels.append("Options"); seg_vals.append(sum(p.get("invested",0) for p in st.session_state.opt_portfolio)); seg_colors.append("#00e5ff")
            if st.session_state.fut_portfolio:
                seg_labels.append("Futures"); seg_vals.append(sum(p.get("margin",0) for p in st.session_state.fut_portfolio)); seg_colors.append("#d500f9")
            if seg_vals:
                fig_pie=go.Figure(data=[go.Pie(labels=seg_labels,values=seg_vals,marker=dict(colors=seg_colors),hole=0.5,textfont=dict(color="#b0c4d8"))])
                fig_pie.update_layout(paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=220,margin=dict(l=10,r=10,t=20,b=10),showlegend=True,legend=dict(font=dict(color="#b0c4d8")))
        ...
        for pos, seg in all_open:
            ...
            port_tbl.append({
                ...
                "Target":f"₹{pos.get('target',0):.2f}","SL":f"₹{pos.get('sl',0):.2f}",
        ...
        if st.button("🛑 SQUARE OFF ALL POSITIONS",use_container_width=True,key="sq_all"):
            for pos in st.session_state.eq_portfolio:
                lp=eng.get_live_price(pos["symbol"]) or pos["entry"]; ep=pos["entry"]; qty=pos["qty"]
                gross=(lp-ep)*qty if pos["type"]=="BUY" else (ep-lp)*qty; net=gross-pos.get("brokerage",0)-eng.equity_cost(lp,qty,pos["type"],False)
                st.session_state.eq_history.append({**pos,"exit":lp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                st.session_state.journal.append({"cat":"EQUITY","symbol":pos["symbol"],"pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
            for pos in st.session_state.opt_portfolio:
                ep=pos["entry"]; cmp=pos.get("cmp",ep); lots=pos["lots"]; ls=pos["lot_size"]
                gross=(cmp-ep)*lots*ls; net=gross-pos.get("brokerage",0)-eng.options_cost(cmp,lots,ls,"SELL")
                st.session_state.opt_history.append({**pos,"exit":cmp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                st.session_state.journal.append({"cat":"OPTIONS","symbol":f"{pos.get('index','')}{pos.get('strike','')}{pos.get('type','')}","pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("signal","")})
            for pos in st.session_state.fut_portfolio:
                lp=eng.get_live_price(pos["symbol"]) or pos["entry"]; ep=pos["entry"]; lots=pos["lots"]; ls=pos["lot_size"]
                gross=(lp-ep)*lots*ls if pos["type"]=="LONG" else (ep-lp)*lots*ls; net=gross-pos.get("brokerage",0)-eng.futures_cost(lp,lots,ls,pos["type"])
                st.session_state.fut_history.append({**pos,"exit":lp,"pnl":round(net,2),"status":"CLOSED","exit_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
                st.session_state.journal.append({"cat":"FUTURES","symbol":pos["symbol"],"pnl":round(net,2),"win":net>=0,"strength":pos.get("strength",0),"date":datetime.now().strftime("%Y-%m-%d"),"rec":pos.get("rec","")})
            st.session_state.eq_portfolio=[]; st.session_state.opt_portfolio=[]; st.session_state.fut_portfolio=[]
            for key in ["eq_portfolio","opt_portfolio","fut_portfolio","eq_history","opt_history","fut_history","journal"]:
                db.save(key,st.session_state[key])
            update_kelly()
            st.success("All positions squared off!"); st.rerun()
...
with page_tabs[4]:
    ...
    all_hist = ([(h,"EQUITY")  for h in st.session_state.eq_history] +
    ...
    if not all_hist:
        ...
        if len(filtered_hist)>=2:
            ...
            fig_cum=go.Figure()
            fig_cum.add_trace(go.Scatter(y=cum_pnl,mode="lines+markers",line=dict(color="#f5a623",width=2),fill="tozeroy",fillcolor="rgba(245,166,35,0.07)",marker=dict(color=["#00e676" if p>=0 else "#ff1744" for p in pnls],size=6)))
            fig_cum.add_hline(y=0,line_dash="dash",line_color="rgba(255,255,255,0.2)")
            fig_cum.update_layout(title="Cumulative Realized P&L",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=280,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"))
            st.plotly_chart(fig_cum,use_container_width=True)

        # P&L by segment bar chart
        if len(all_hist)>=2:
            seg_pnl={"EQUITY":0,"OPTIONS":0,"FUTURES":0}
            for h,s in all_hist: seg_pnl[s]=seg_pnl.get(s,0)+h.get("pnl",0)
            fig_seg=go.Figure(data=[go.Bar(x=list(seg_pnl.keys()),y=list(seg_pnl.values()),marker_color=["#00e676" if v>=0 else "#ff1744" for v in seg_pnl.values()],text=[f"₹{v:+,.0f}" for v in seg_pnl.values()],textposition="auto")])
            fig_seg.update_layout(title="P&L by Segment",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=240,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"))
            st.plotly_chart(fig_seg,use_container_width=True)

        all_df=pd.DataFrame(hist_rows)
        st.download_button("📥 Download Full History CSV",data=all_df.to_csv(index=False),file_name=f"full_history_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")
...
with page_tabs[5]:
    ...
    if not jrnl:
        ...
        st.markdown(f'<div class="success-b">📒 {len(jrnl)} journal entries loaded from persistent storage. This data survives page reloads and session restarts.</div>',unsafe_allow_html=True)
        st.markdown("<br>",unsafe_allow_html=True)
        ...
        if "cat" in jdf.columns:
            ...
            cat_agg=jdf.groupby("cat").agg(trades=("pnl","count"),net_pnl=("pnl","sum"),win_rate=("win","mean")).reset_index()
            cat_agg["win_rate"]=(cat_agg["win_rate"]*100).round(1)
            st.dataframe(cat_agg.rename(columns={"cat":"Segment","trades":"Trades","net_pnl":"Net P&L(₹)","win_rate":"Win Rate%"}),use_container_width=True,hide_index=True)
        ...
        if "strength" in jdf.columns:
            ...
            bagg=jdf2.groupby("bucket",observed=True).agg(trades=("pnl","count"),net_pnl=("pnl","sum"),win_rate=("win","mean")).reset_index()
            bagg["win_rate"]=(bagg["win_rate"]*100).round(1)
            fig_b=px.bar(bagg,x="bucket",y="net_pnl",color="win_rate",color_continuous_scale=["#ff1744","#ffd600","#00e676"],title="P&L by Signal Strength Bucket",labels={"bucket":"Signal Strength","net_pnl":"Net P&L (₹)"})
            fig_b.update_layout(paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=260,margin=dict(l=40,r=20,t=30,b=20))
        ...
        if "date" in jdf.columns:
            ...
            fig_d=go.Figure(data=[go.Bar(x=daily["date"],y=daily["pnl"],marker_color=["#00e676" if v>=0 else "#ff1744" for v in daily["pnl"]],text=[f"₹{v:+,.0f}" for v in daily["pnl"]],textposition="auto")])
            fig_d.update_layout(title="Daily P&L",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=260,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"))
        ...
        st.download_button("📥 Download Journal CSV",data=pd.DataFrame(jrnl).to_csv(index=False),file_name=f"trade_journal_{datetime.now().strftime('%Y%m%d')}.csv",mime="text/csv")

        if st.button("🗑️ Clear Journal (PERMANENT)",key="clr_jrnl"):
            st.session_state.journal=[]; st.session_state.kelly_wr=0.55
            db.save("journal",[]); db.save("kelly_wr",0.55); st.rerun()
...
with page_tabs[6]:
    ...
    all_closed = (st.session_state.eq_history +
    ...
    if len(all_closed) < 2:
        ...
        fig_eq=go.Figure()
        fig_eq.add_trace(go.Scatter(y=pnl_series,mode="lines",name="Equity",line=dict(color="#f5a623",width=2.5),fill="tozeroy",fillcolor="rgba(245,166,35,0.06)"))
        fig_eq.add_trace(go.Scatter(y=rolling_max,mode="lines",name="Peak",line=dict(color="#00e5ff",width=1,dash="dash")))
        fig_eq.add_hline(y=0,line_dash="dash",line_color="rgba(255,255,255,0.15)")
        fig_eq.update_layout(title="Equity Curve (Cumulative P&L)",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=300,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"),legend=dict(font=dict(color="#b0c4d8")))
        st.plotly_chart(fig_eq,use_container_width=True)

        # ── Drawdown Chart ────────────────────────────────────────────────────
        fig_dd=go.Figure()
        fig_dd.add_trace(go.Scatter(y=drawdown,mode="lines",fill="tozeroy",fillcolor="rgba(255,23,68,0.1)",line=dict(color="#ff1744",width=1.5)))
        fig_dd.update_layout(title="Drawdown",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=200,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"))
        ...
        fig_dist.add_trace(go.Histogram(x=pnl_vals,nbinsx=30,marker_color=["#00e676" if v>=0 else "#ff1744" for v in pnl_vals],opacity=0.75,name="P&L Distribution"))
        fig_dist.add_vline(x=0,line_dash="dash",line_color="rgba(255,255,255,0.3)")
        fig_dist.update_layout(title="P&L Distribution Histogram",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=260,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(gridcolor="#1a2d45"),yaxis=dict(gridcolor="#1a2d45"))
        st.plotly_chart(fig_dist,use_container_width=True)

        # ── Strength vs P&L scatter ───────────────────────────────────────────
        ...
        fig_sc=go.Figure()
        fig_sc.add_trace(go.Scatter(x=str_vals,y=pnl_vals,mode="markers",marker=dict(color=pnl_col,size=9,opacity=0.8),text=[t.get("symbol","") for t in all_closed],hovertemplate="<b>%{text}</b><br>Str: %{x}%<br>P&L: ₹%{y:,.0f}"))
        fig_sc.add_hline(y=0,line_dash="dash",line_color="rgba(255,255,255,0.2)")
        fig_sc.update_layout(title="Signal Strength vs P&L",paper_bgcolor="#080c14",plot_bgcolor="#080c14",font=dict(color="#b0c4d8"),height=280,margin=dict(l=40,r=20,t=30,b=20),xaxis=dict(title="Signal Strength %",gridcolor="#1a2d45"),yaxis=dict(title="Net P&L (₹)",gridcolor="#1a2d45"))
        ...
        with col_w:
            ...
            top_wins=sorted(all_closed,key=lambda x:-x.get("pnl",0))[:10]
            for t in top_wins:
                sym=t.get("symbol","") or f"{t.get('index','')}{t.get('strike','')}{t.get('type','')}"
                st.markdown(f'<div class="jrnl-row"><span style="font-family:JetBrains Mono;font-size:0.8rem;">{sym.replace(".NS","")}</span><span class="pnl-pos">₹{t.get("pnl",0):+,.0f}</span></div>',unsafe_allow_html=True)
        with col_l:
            st.markdown("#### ❌ Top 10 Losers")
            top_loss=sorted(all_closed,key=lambda x:x.get("pnl",0))[:10]
            for t in top_loss:
                sym=t.get("symbol","") or f"{t.get('index','')}{t.get('strike','')}{t.get('type','')}"
                st.markdown(f'<div class="jrnl-row"><span style="font-family:JetBrains Mono;font-size:0.8rem;">{sym.replace(".NS","")}</span><span class="pnl-neg">₹{t.get("pnl",0):+,.0f}</span></div>',unsafe_allow_html=True)
        ...
        # ── Kelly Win Rate update display ─────────────────────────────────────
        st.markdown("<br>",unsafe_allow_html=True)
        st.markdown(f'<div class="info-b">🧮 <b>Kelly Criterion Win Rate: {st.session_state.kelly_wr*100:.1f}%</b> (from {len(all_closed)} trades) — Used for dynamic position sizing across all segments.<br>Avg Win/Loss Ratio: {pf_a:.2f}x | Expected Value per trade: ₹{total_pnl_a/max(len(all_closed),1):,.0f}</div>',unsafe_allow_html=True)

# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown("<br>",unsafe_allow_html=True)
st.markdown(f"""<div style="background:var(--bg2);border-top:1px solid var(--border);padding:12px 20px;text-align:center;font-family:'JetBrains Mono';font-size:0.65rem;color:var(--muted);">
